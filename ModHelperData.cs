namespace OPLegendOfTheNight;

public static class ModHelperData
{
    public const string WorksOnVersion = "34.3";
    public const string WorksOnVersion = "36.3";
    public const string Version = "1.0.2";
    public const string Name = "OP Legend Of The Night";
    public const string Description = "This mod makes the legend of the night portal suck up anything that leaks, and I mean ANYTHING. You can change the price of the upgrade in mod settings.";
    public const string RepoName = "OPLegendOfTheNight";
    public const string Icon = "OPLegendOfTheNightIcon.png";
}
